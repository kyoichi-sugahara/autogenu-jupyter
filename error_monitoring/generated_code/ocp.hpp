...
public:
  ...
  void eval_hx(const double t, const double *x, const double *u,
    ...
    hx[1] = -lmd[2] * x10 * (u[0] + x1 * x5) +
            lmd[2] * x11 * (x2 * x5 + x8 * (x12 - x13)) +
            (1.0 / 2.0) * q[1] * (x0 - 2 * x_ref[1]) -
            x10 * x15 * (-u[0] * x2 - x13 * x14 - x4 * x9) +
            x11 * x15 *
                (-m_p * pow(x2, 2) * x4 + u[0] * x1 - x12 * x9 - x14 * x3 +
                 x4 * x6);
