...
初期推定解を$x_0$、最終的な解を$x$とすると、差のノルムは以下のように定義します。

$$ ||x - x_0|| = \sqrt{ (x_1 - x_{1_0})^2 + (x_2 - x_{2_0})^2 + \cdots + (x_n - x_{n_0})^2 } $$

ここで、$x_1, x_2, \ldots, x_n$は$x$の要素であり、$x_{1_0}, x_{2_0}, \ldots, x_{n_0}$は$x_0$の対応する要素です。すべての要素の差の2乗を加算し、その平方根を取ることで、ノルムを求めることができます。

### 解のスケールのばらつき

$$CV = \frac{\sigma}{\mu} = \frac{\sqrt{\frac{\sum_{i=1}^{N}(x_i-\mu)^2}{N}}}{\frac{\sum_{i=1}^{N}x_i}{N}}$$

ここで、CVはCoefficient of Variationを表します。また、$\sigma$は標準偏差、$\mu$は平均値、$x_i$は各データポイント、$N$はサンプルサイズを表します。上記式は、標準偏差を平均値で割り、100倍したものによって相対的な散らばりを評価する方法を示しています。


標準偏差を平均値で割った値で計算され、データの散らばりを相対的に評価することができます。CV は、データの単位やスケールに依存しないため、異なるデータセットや異なる単位のデータを比較する際に有用である。

データの平均値が非常に小さい場合、CV は不安定になることがあるため、注意が必要そう。
...
$x_i = x_{normalized_i} * (\max(x) - \min(x)) + \min(x)$

ここで、元のスケールに戻したい解の$i$番目の成分を$x_i$、正規化された解のi番目の成分を$x_{normalized_i}$、解の成分の最小値を$\min(x)$、解の成分の最大値を$\max(x)$と表します。
