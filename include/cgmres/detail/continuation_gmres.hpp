...
template <class NLP> class ContinuationGMRES {
public:
  ...
  ContinuationGMRES(const NLP &nlp, const Scalar finite_difference_epsilon,
                    const Scalar zeta)
      : nlp_(nlp), finite_difference_epsilon_(finite_difference_epsilon),
        zeta_(zeta), updated_solution_(Vector<dim>::Zero()),
        fonc_(Vector<dim>::Zero()), fonc_1_(Vector<dim>::Zero()),
        fonc_2_(Vector<dim>::Zero()), x_1_(Vector<nx>::Zero()),
        dx_(Vector<nx>::Zero()) {
  ...
  Scalar optError() const { return fonc_.template lpNorm<2>(); }

  template <typename VectorType>
  void eval_fonc(const Scalar t, const MatrixBase<VectorType> &x,
                 const Vector<dim> &solution, const bool debug = false) {
    nlp_.eval_fonc_hu(t, x, solution, fonc_, debug);
  }

  /// @brief TODO(Sugahara): Write the description of this function.
  /// @tparam VectorType1
  /// @tparam VectorType2
  /// @tparam VectorType3
  /// @tparam VectorType4
  ...
  void eval_b(const Scalar t, const MatrixBase<VectorType1> &x,
    ...
    nlp_.ocp().eval_f(t, x.derived().data(), solution.derived().data(),
                      dx_.data());
    ...
    CGMRES_EIGEN_CONST_CAST(VectorType4, b_vec) =
        (1 / finite_difference_epsilon_ - zeta_) * fonc_ -
  ...
  void eval_Ax(const Scalar t, const MatrixBase<VectorType1> &x,
    ...
    updated_solution_ = solution + finite_difference_epsilon_ * solution_update;
    nlp_.eval_fonc_hu(t1, x_1_, updated_solution_, fonc_2_);
    CGMRES_EIGEN_CONST_CAST(VectorType4, ax_vec) =
        (fonc_2_ - fonc_1_) / finite_difference_epsilon_;
  }
