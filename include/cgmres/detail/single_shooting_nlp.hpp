...
template <class OCP, int N> class SingleShootingNLP {
public:
  ...
  SingleShootingNLP(const OCP &ocp, const Horizon &horizon)
      : ocp_(ocp), horizon_(horizon), dx_(Vector<nx>::Zero()) {
  ...
  void eval_fonc_hu(const Scalar t, const MatrixBase<VectorType> &x,
                    const Vector<dim> &solution, Vector<dim> &fonc_hu,
    ...
    ocp_.eval_f(t, x_[0].data(), solution.template head<nuc>().data(),
                dx_.data());
    x_[1] = x_[0] + dt * dx_;
    for (size_t i = 1; i < N; ++i) {
      const int inucb2 = i * (nuc + 2 * nub);
      ocp_.eval_f(t + i * dt, x_[i].data(),
                  solution.template segment<nuc>(inucb2).data(), dx_.data());
      x_[i + 1] = x_[i] + dt * dx_;
    }
    ...
    for (size_t i = N - 1; i >= 1; --i) {
      const int inucb2 = i * (nuc + 2 * nub);
      ocp_.eval_hx(t + i * dt, x_[i].data(),
                   solution.template segment<nuc>(inucb2).data(),
                   lmd_[i + 1].data(), dx_.data());
      lmd_[i] = lmd_[i + 1] + dt * dx_;
    }
    // Compute the erros in the first order necessary conditions (FONC)
    ocp_.eval_hu(t, x_[0].data(), solution.template head<nuc>().data(),
                 lmd_[1].data(), fonc_hu.template head<nuc>().data());
    for (size_t i = 1; i < N; ++i) {
      const int inucb2 = i * (nuc + 2 * nub);
      ocp_.eval_hu(t + i * dt, x_[i].data(),
                   solution.template segment<nuc>(inucb2).data(),
                   lmd_[i + 1].data(),
                   fonc_hu.template segment<nuc>(inucb2).data());
    }
  ...
  void retrieve_dummy(Vector<dim> &solution, Vector<dim> &fonc_hu,
