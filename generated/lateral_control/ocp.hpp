...
#ifndef CGMRES__OCP_LATERAL_CONTROL_HPP_ 
#define CGMRES__OCP_LATERAL_CONTROL_HPP_ 
...
public:
  ...
  void disp(std::ostream& os) const {
    ...
    os << "  smoothed_curvature_in_reference_trajectory: " << smoothed_curvature_in_reference_trajectory << std::endl;
  ...
  void eval_f(const double t, const double* x, const double* u, 
    ...
    dx[1] = -curvature_in_reference_trajectory*v_in_reference_trajectory*cos(x[1]) + v_in_reference_trajectory*tan(x[2])/wheel_base;
    dx[2] = -(-u[0] + x[2])/τ;
  ...
  void eval_hx(const double t, const double* x, const double* u, 
    ...
    hx[1] = curvature_in_reference_trajectory*x0*sin(x[1]) + lmd[0]*v_in_reference_trajectory*cos(x[1]) + (1.0/2.0)*q[1]*(2*x[1] - 2*x_ref[1]);
    hx[2] = -lmd[2]/τ + (1.0/2.0)*q[2]*(2*x[2] - 2*x_ref[2]) + x0*(pow(tan(x[2]), 2) + 1)/wheel_base;
