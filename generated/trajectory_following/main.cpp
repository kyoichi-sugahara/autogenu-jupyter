...
int main() {
  ...
  cgmres::ZeroHorizonOCPSolver<cgmres::OCP_trajectory_following, kmax_init>
      initializer(ocp, settings);
  cgmres::Vector<1> uc0;
  uc0 << 0.0;
  initializer.set_uc(uc0);
  initializer.solve(t0, x0);
  std::cerr << "u =: " << initializer.ucopt()[0] << std::endl;

  std::cerr << "u = : " << initializer.uopt() << std::endl;

  // Define the C/GMRES solver.
  constexpr int N = 50;
  constexpr int kmax = 5;
  cgmres::SingleShootingCGMRESSolver<cgmres::OCP_trajectory_following, N, kmax>
      mpc(ocp, horizon, settings);
  ...
  for (unsigned int i = 0; i < sim_steps; ++i) {
    ...
    const auto &u =
        mpc.uopt()[0]; // const reference to the initial optimal control input
    // const cgmres::VectorX x1 = cgmres::RK4(ocp, t, sampling_time, x, u);  //
    // the next state
    mpc.update(t, x); // update the MPC solution

    mpc.optError(t, x, settings.verbose_level); // compute the optimal error
    logger.save(t, x, u, mpc.uopt(), mpc.optError(), mpc.normDiff(),
                mpc.StandardDeviation());
    // x = x1;
