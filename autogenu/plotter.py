...
class Plotter(object):
    ...
    def __init__(self, log_dir, log_name: str):
        ...
        self.__standard_deviation_data = np.genfromtxt(os.path.join(log_dir, log_name+'_standard_deviation.log'))
        # Replace NaN with 0.
        ...
        self.__standard_deviation_data[np.isnan(self.__standard_deviation_data)] = 0
    ...
    def __plot(self):
        ...
        for i in range(len(self.__t_data)):
            if self.__standard_deviation_data[i] < cut_off and self.__standard_deviation_data[i] > -cut_off:
                x.append(self.__t_data[i])
                y.append(self.__standard_deviation_data[i])

        # グラフの描画
        plt.plot(x, y)
        # plt.plot(self.__t_data, self.__standard_deviation_data)
        plt.xlabel(r'${\rm Time}$ $[s]$')
